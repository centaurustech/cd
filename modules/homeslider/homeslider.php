...
class HomeSlider extends Module
{
	...
	private function _displayForm()
	{
		...
		<fieldset>
			...
			<strong>
				<a href="index.php?tab='.Tools::getValue('tab').'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&addSlide">
				...
				$this->_html .= '
  					<li id="slides_'.$slide['id_slide'].'">
    					...
    					<p style="float: right">'.
    						...
    						<a href="index.php?tab='.Tools::getValue('tab') .'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&id_slide='.(int)($slide['id_slide']).'" title="'.$this->l('Edit').'"><img src="'._PS_ADMIN_IMG_.'edit.gif" alt="" /></a>
							<a href="index.php?tab='.Tools::getValue('tab').'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&delete_id_slide='.(int)($slide['id_slide']).'" title="'.$this->l('Delete').'"><img src="'._PS_ADMIN_IMG_.'delete.gif" alt="" /></a>
						</p>
  					</li>';
			}
			$this->_html .= '</ul></div>';
	...
	private function _displayAddForm()
	{
		...
		<p class="center">
			...
			<a class="button" style="position:relative; padding:3px 3px 4px 3px; top:1px" href="index.php?tab='.Tools::getValue('tab').'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">'.$this->l('Cancel').'</a>
		</p>';
	...
	private function _postProcess()
	{
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			foreach ($languages as $language)
			{
				...
				{
					if ($error = checkImage($_FILES['image_'.$language['id_lang']], 2000000))
						$errors .= $error;
	...
	public function hookHome()
	{
		...
		$smarty->assign('homeslider_slides', $slides);
		$smarty->assign('homeslider', $slider);
	...
	public function hookHeader($params)
	{
		...
		Tools::addJS($this->_path.'js/jquery.bxSlider.min.js');
		Tools::addCSS($this->_path.'bx_styles.css');
	}
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			...
			WHERE hs.`id_slide` = hss.`id_slide` AND hss.`id_slide` = hssl.`id_slide` AND hs.`id_slide` = hssl.`id_slide`
	...
	public function displayStatus($id_slide, $active)
	{
		...
		$html = '<a href="index.php?tab='.Tools::getValue('tab').'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&changeStatus&id_slide='.(int)($id_slide).'" title="'.$title.'"><img src="'._PS_ADMIN_IMG_.''.$img.'" alt="" /></a>';
		return $html;
	}
