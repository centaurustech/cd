...
final class Log {
	...
	public function write($message) {
		$file = _PS_MODULE_DIR_.'payline/logs/' . $this->filename;
		$handle = fopen($file, 'a+');
...
class paylineSDK{
	...
	// kit version
	const KIT_VERSION		= 'module Prestashop v1.2';
	...
	function __construct($merchant_id, $acess_key, $proxy_host, $proxy_port, $proxy_login, $proxy_password, $production) {
		...
		$this->wsdlPath = _PS_MODULE_DIR_.'payline/lib/';
	...
	*/
	...
	private function webServiceRequest($array,$WSRequest,$PaylineAPI,$Method){
		try{
			...
			$DateDebut = time();
			$this->VerifyIfAnotherWShasSwitch($Method);
			$client = new SoapClient($this->wsdlPath.paylineSDK::WSDL, $this->header_soap);
