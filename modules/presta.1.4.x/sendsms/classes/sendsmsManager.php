...
class sendsmsManager
{
	...
	public function l($string, $lang_id=null)
	{
		...
		$currentKey = '<{'.$this->_module.'}prestashop>'.strtolower(get_class($this)).'_'.md5($string2);
	...
	private function _getUpdateQuantityValues($bSimu = false, $bAdmin = false)
	{
		if ($bSimu) {
			...
			{
				$product['name'] = $product['name'][(int)Configuration::get('PS_LANG_DEFAULT')];
			}
			if (isset($product['id_product']))
				$product['id'] = (int)$product['id_product'];

			$quantity = $product['quantity'];
			if ($quantity <= intval(Configuration::get('PS_LAST_QTIES')) AND Configuration::get('PS_STOCK_MANAGEMENT'))
			{
				$values = array(
					'{product_id}' => intval($product['id']),
					'{product_ref}' => strval($product['reference']),
					'{product_name}' => strval($product['name']),
					'{quantity}' => $quantity
				);
			} else
				return false;
