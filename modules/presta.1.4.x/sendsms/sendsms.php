...
class sendsms extends Module
{
	...
	private function _installDatabase() {
		...
		Db::getInstance()->Execute(
			'ALTER TABLE `' . _DB_PREFIX_ . 'sendsms_logs` ENGINE=InnoDB;'
		);
		...
		Db::getInstance()->Execute(
			'ALTER TABLE `' . _DB_PREFIX_ . 'sendsms_phone_prefix` ENGINE=InnoDB;'
		);
		...
		Db::getInstance()->Execute("
			INSERT INTO `" . _DB_PREFIX_ . "sendsms_phone_prefix` (`iso_code`, `prefix`) VALUES
				...
				('ET', 251),('FI', 358),('FJ', 679),('FK', 500),('FM', 691),('FO', 298),('FR', 33),('GA', 241),('UK', 44),('GD', 1473),
	...
	private function _installHooks()
	{
		...
		if (!$this->registerHook('createAccount') || !$this->registerHook('newOrder') || !$this->registerHook('updateQuantity') || !$this->registerHook('orderReturn')
		 ...
		 || !$this->registerHook('sendsmsCustomerChoice') || !$this->registerHook('sendsmsProcessCustomerChoice')
	...
	private function _installFiles() {
		$this->_modifyFile('./tabs/AdminOrders.php', 'sendsmsShippingNumber', "@Mail::Send((int)(\$order->id_lang), 'in_transit', ((is_array(\$_LANGMAIL) AND key_exists(\$subject, \$_LANGMAIL)) ? \$_LANGMAIL[\$subject] : \$subject), \$templateVars, \$customer->email, \$customer->firstname.' '.\$customer->lastname);", "@Mail::Send((int)(\$order->id_lang), 'in_transit', ((is_array(\$_LANGMAIL) AND key_exists(\$subject, \$_LANGMAIL)) ? \$_LANGMAIL[\$subject] : \$subject), \$templateVars, \$customer->email, \$customer->firstname.' '.\$customer->lastname);\nModule::hookExec('sendsmsShippingNumber', array('customer' => \$customer, 'order' => \$order, 'carrier' => \$carrier));");
		$this->_modifyFile('./tabs/AdminOrders.php', 'sendsmsShippingNumber', "@Mail::Send((int)(\$order->id_lang), 'in_transit', Mail::l('Package in transit'), \$templateVars, \$customer->email, \$customer->firstname.' '.\$customer->lastname);", "@Mail::Send((int)(\$order->id_lang), 'in_transit', Mail::l('Package in transit'), \$templateVars, \$customer->email, \$customer->firstname.' '.\$customer->lastname);\nModule::hookExec('sendsmsShippingNumber', array('customer' => \$customer, 'order' => \$order, 'carrier' => \$carrier));");
		...
		$this->_modifyFile('../modules/mailalerts/mailalerts.php', 'sendsmsCustomerAlert', "\$customer_id = (int)\$customer->id;", "\$customer_id = (int)\$customer->id;\nModule::hookExec('sendsmsCustomerAlert', array('customer' => \$customer, 'product' => \$product));");
